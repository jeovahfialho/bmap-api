...
import { Card, ProcessedCard, UserLoggedTime } from '../domain/entities/Card';
...
export class GetInitiativesWithHistories implements CardsService {
  ...
  async getInitiativesWithHistories(filters?: Record<string, any>): Promise<{ processedCards: ProcessedCard[], pagination: { all_pages: number, current_page: number, results_per_page: number } }> {
    ...
    try {
      ...
      const boardIds = filters?.board_ids?.map((id: string) => Number(id)) || [];
      ...
      const initiatives = allCards.filter(card => {
        if (card.type_id !== 2) return false;
        if (boardIds.length > 0 && card.board_id && !boardIds.includes(card.board_id)) return false;
        return true;
      });
      ...
      const processedInitiatives = await Promise.all(
        initiatives.map(initiative => this.processInitiativeWithChildren(initiative, allCards, userMap))
      );
  ...
  private aggregateLoggedTimesByUser(card: Card, userMap: Map<number, string>): UserLoggedTime[] {
    const loggedTimes = card.logged_times || [];
    if (loggedTimes.length === 0) return [];

    const aggregated = new Map<number, { total_time: number; entries_count: number }>();
  ...
  private async processInitiativeWithChildren(initiative: Card, allCards: Card[], userMap: Map<number, string>): Promise<ProcessedCard> {
